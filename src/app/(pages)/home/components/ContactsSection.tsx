...
export default function ContactsSection() {
  ...
  const handleSelectContact = (contact: IConversation) => {
    const contactFound = contactsSelected.findIndex(
      (contactSelected) => contactSelected.id === contact.id
    );

    if (contactFound !== -1) {
      const newContactsSelected = contactsSelected.filter(
        (c, i) => i !== contactFound
      );
      ...
      <div className="p-4 space-y-6 h-full overflow-hidden mb-6 ">
        <div
          className={`flex justify-between items-center ${
            contactsList.length > 1 ? "w-[500px]" : "w-[300px]"
          } space-x-4`}
        >
          ...
          <div className="bodyChat overflow-y-auto scrollbar-custom p-4">
            <div className="flex flex-wrap gap-4 w-full">
              {contactsSelected.map((contact, index) => (
                <div
                  key={index}
                  className="p-1 pe-4 rounded-full w-fit border flex gap-2 hover:scale-105 transition-all duration-300 hover:bg-gray-100"
                >
                  <img
                    src={`https://ui-avatars.com/api/?name=${contact!.contactName?.replaceAll(
                      " ",
                      "+"
                    )}&background=random`}
                    alt="contractor"
